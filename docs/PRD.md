...
**Function inventory is small and closed.** The Phase 1 whole-corpus scan (all 97 files, 1,743,786 formulas; string literals and quoted sheet names stripped to avoid false positives) found exactly **54 distinct functions**: the sample's 44 — IF (174k uses in the sample alone), SIN, SQRT, COS, AND, ABS, EXP, PI, CONCATENATE, VLOOKUP, INDIRECT, COSH, MATCH, INDEX, MAX, SINH, MIN, FALSE, MOD, CEILING, OR, INT, ASIN, AVERAGE, LOG, TRUE, SUM, ROUND, ISNA, MID, ROUNDUP, NOT, FLOOR, ADDRESS, TEXT, HLOOKUP, ISERROR, SUMSQ, ATAN, ISNUMBER, SEARCH, FIXED, LEN, VALUE — plus ten from the remaining 83 files: LN, TAN, LOG10, ROUNDDOWN, COUNT, ISERR, ERROR.TYPE, OFFSET (single use, `OFFSET(name, 0, computed)` feeding a VLOOKUP), and the array pair MMULT/MINVERSE. Dominated by scalar math/trig and IF-chains; dynamic arrays and financial/date functions are absent.

**Array formulas exist in exactly one file.** ACDC_LYTSwitch1_BuckBoost_Rev1 contains six identical legacy CSE blocks, each `{=MMULT(MINVERSE(7×7), 7×1)}` spilled over a 7×1 block (polynomial coefficient solve). No other array formulas, **no VBA, no Excel tables, no pivot tables, no live external references** anywhere in the corpus (one legacy `externalLink` part exists in TNY-4 but no cell references it).
